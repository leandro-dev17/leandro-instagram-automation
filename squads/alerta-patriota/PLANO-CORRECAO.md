...
**Item 6 — Mini-auditoria dos achados sem dono da Fase 30 (Agentes Fiscais/Gestão): ✅ CONCLUÍDO (implementado, aguardando deploy)**

A Fase 30 citava "5 achados adicionais" e "9 achados de baixa severidade" na categoria Fiscais/Gestão sem nunca nomeá-los no documento. Como checar os ~31 arquivos candidatos um por um seria caro de contexto, a investigação foi delegada a um subagente Explore com um briefing das 8 classes de bug já conhecidas nessa categoria (catch silencioso, fetch só validado por exceção/não por `res.ok`, função de auto-correção nunca chamada, heurística `LIKE`/`ILIKE` frágil, contador/score sem teto, alerta sem dedup, rota lenta sem `maxDuration`, gap de observabilidade). Achados confirmados manualmente (2 deles verificados via grep/leitura direta antes de apresentar a lista ao usuário) e corrigidos em lote único, por decisão do usuário ("Corrigir todos agora — lote único"):

1. **`fiscal-banco.ts`** — bug circular: o catch que trata "banco fora do ar" chamava `criarAlertaDedup()`, que também consulta o banco — se o banco estivesse mesmo fora do ar, o dedup falhava junto e o `.catch(() => ({ criado: false }))` original fazia o código achar que já tinha alertado, silenciando justo o cenário mais crítico que esse catch existe para cobrir. Corrigido invertendo o fallback para `criado: true` (aceita risco de alerta duplicado durante a queda em troca de nunca ficar em silêncio total).
2. **`fiscal-api.ts`** — mesmo bug circular, no catch da checagem de rotas críticas (`/api/auth/me`, `/api/admin/stats`, `/`). Mesma correção.
3. **`fiscal-pagamentos.ts`** — mesmo bug circular, no catch geral da rota. Mesma correção.
4. **`gerente-clientes.ts`** — score `100` só decrementava (cancelamentos, queda de membros, inativos, Bot Responder), sem teto inferior — mesmo bug já corrigido em `gerente-financeiro.ts` (Item 20, Fase 32), não replicado aqui. Adicionado `score = Math.max(0, Math.min(100, score))` antes do escalonamento.
5. **`gerente-conteudo.ts`** — mesma classe de bug do item 4 (score sem clamp), mais 3 blocos com `catch { /* silencioso */ }` que não somavam ao score nem ao relatório de problemas — uma falha real de query era indistinguível de "tudo certo". Adicionado clamp + os 3 catches agora registram o erro em `problemas` e descontam 5 pontos, como os demais blocos do arquivo já faziam.
6. **`gerente-tecnico.ts`** — o achado original (checagem do Agente Médico só pelo status, sem idade) já tinha sido corrigido na Fase 27.5 (comentário no próprio código confirma) — falso positivo do levantamento da Fase 30. Em vez disso, confirmado e corrigido o mesmo bug de score sem clamp dos itens 4/5 (aqui o risco é maior: um único bloco pode descontar até 45 pontos de uma vez).
7. **`fiscal-apis-externas.ts`** — contagem de falhas consecutivas usava `ILIKE %nome%` sobre um campo de texto livre (`api_down`, nomes separados por vírgula) — frágil a nomes que sejam substring um do outro. Trocado por `detalhes->'downs' ? nome`, comparando contra o array jsonb exato já gravado, sem risco de match parcial.
8. **`fiscal-fontes.ts`** — `buscarUltimaNoticia()` usava `ILIKE %nomeFonte%` contra `noticias.fonte`, que é gravado com o nome exato (`coletar-noticias.ts`). Isso fazia "Jovem Pan" (fonte RSS) dar match em notícias de "Jovem Pan News" (canal do YouTube, fonte separada), escondendo o feed RSS realmente parado. Trocado para `=` (igualdade exata).
9. **`fiscal-conteudo.ts`** — `contemPalavraProibida()` usava `.includes()` puro: a palavra-filtro "gol" (contexto futebol) dava match dentro de "golpe" — termo político central ("golpe militar", "ameaça de golpe") que este fiscal existe justamente para **não** filtrar. Reescrito com lookaround simulando limite de palavra (incluindo acentos), sem depender de `\b`/flag `u`.
10. **`fiscal-qualidade-resumo.ts`** — validação da assinatura do Prof. Cavalcanti aceitava os fragmentos soltos "mundo muda" ou "enxerga antes" em qualquer ponto do texto, não só a frase completa da assinatura real ("O mundo muda para quem enxerga antes.") — um resumo sem assinatura de verdade, mas que mencionasse esses termos no corpo, passava como válido. Corrigido exigindo a frase completa e restringindo a busca aos últimos 200 caracteres do texto (onde a assinatura sempre fica).
11. **`fiscal-codigo-logica.ts`** — a escalação para `revisor-logica` (quando há problema de lógica) era `await fetch(...).catch(() => {})` — uma falha nessa chamada (timeout, 5xx) ficava muda; o problema original já tinha sido alertado, mas ninguém saberia que o Revisor de Lógica nunca chegou a rodar. Adicionado registro em `agentes_log` quando a escalação falha ou retorna não-OK.
12. **`termometro.ts`** — os dois fallbacks de `gerarTermometro()` (resposta da IA sem JSON / JSON inválido) eram completamente mudos: o termômetro postava scores fixos genéricos (`democracia: 4, economia: 4...`) para VIP e Elite todo domingo sem nenhum log ou alerta indicando que a geração real falhou. Adicionada flag `parseFalhou` propagada até o handler: dispara alerta Telegram (🟡) e grava `status: 'aviso'` em `agentes_log` quando o fallback é usado, mantendo o post (não dá para simplesmente abortar um conteúdo semanal).

`tsc --noEmit`: 0 erros novos em todos os 9 arquivos tocados (mesmo erro pré-existente em `admin/usuarios/[id]/route.ts`, não relacionado a esta sessão).
...
## FASE 34 — Bug urgente reportado pelo usuário: duplicação no Elite + conteúdo do Brasil no grupo global

Investigação do backlog de segurança/infra (itens adiados nas Fases 21-23, escolhido pelo usuário como próximo passo após a Fase 33) foi interrompida por um bug urgente reportado: "No grupo whatsapp Elite tem a postagem programada chamada ANÁLISE URGENTE - ela esta sendo duplicada sempre que posta, [...] sem contar que o grupo Elite não deveria ter postagens e comentários de empresários e deputados brasileiros, esse grupo é global deve ter comentários e postagens da direita mundial, empresários, presidentes, deputados de direita do mundo menos Brasil." Backlog de segurança/infra retomado depois desta fase.

Dois bugs reais confirmados em `radar-politico/route.ts` (AGENTE RAQUEL RADAR + VICTOR VIRAL):

1. **Duplicação da ANÁLISE URGENTE no Elite.** O dedup existente só comparava `url` exata (`ON CONFLICT (url)`). Quando a mesma declaração de um político saía simultaneamente como vídeo do canal do YouTube e como matéria de portal/mídia (URLs diferentes para o mesmo evento real), as duas menções passavam o dedup e geravam duas análises seguidas, postadas em sequência.
2. **Conteúdo do Brasil indo para o grupo Elite (que é global).** O arquivo gerava duas personas por menção: Capitão Braga (postava no VIP) e Prof. Cavalcanti (postava no Elite, com "ângulo global/econômico"). Mas a lista `PESSOAS` monitorada era 100% de políticos e empresários brasileiros (Nikolas Ferreira, Eduardo Bolsonaro, Sergio Moro, Luciano Hang, Flávio Augusto, Pablo Marçal etc.) — o "ângulo global" do Cavalcanti era só um tom de texto, não mudava o assunto. Resultado real: o grupo Elite, que deveria ter direita mundial/presidentes/empresários estrangeiros (já servido corretamente por `resumir-noticias-global.ts`, pipeline separado e correto), recebia comentários sobre políticos brasileiros — confundindo VIP e Elite.

Fix escolhido pelo usuário entre as opções apresentadas (com investigação prévia de qual pipeline já cobria conteúdo global de verdade):

1. **Dedup por pessoa+dia, além do dedup por URL**: nova query batched (`SELECT politico FROM radar_politico WHERE politico = ANY(...) AND processado = true AND created_at >= NOW() - INTERVAL '3 hours' GROUP BY politico`), no mesmo padrão de batching já usado por `contagensPorPessoa` (Fase 30/Item 22) — evita N+1. Antes de gerar a análise de uma pessoa, agora checa também se ela já teve uma análise processada nas últimas 3h, independente da URL.
2. **Removido Prof. Cavalcanti/Elite deste arquivo por completo** — já existe `resumir-noticias-global.ts` com `PROMPT_CAVALCANTI_GLOBAL` servindo o Elite corretamente com conteúdo genuinamente internacional; não fazia sentido manter um segundo caminho postando Brasil no mesmo grupo. Removida a função `gerarAlertaCavalcanti`, o bloco de postagem no Elite (fetch a `grupos_whatsapp WHERE plano='elite'` + `enviarMensagemGrupo("elite", ...)` + insert em `posts_whatsapp`), a coluna `resumo_cavalcanti` deste insert específico, e o campo `tipo: "empresario" | "politico"` do tipo `Pessoa` (não tinha mais função sem o Cavalcanti, já que Capitão Braga nunca comenta empresários). Os 3 empresários (Luciano Hang, Flávio Augusto, Pablo Marçal) também foram removidos da lista `PESSOAS` — monitorá-los sem o Cavalcanti não geraria mais nenhum post, então ficariam como monitoramento morto. `PESSOAS` ficou só com os 6 políticos brasileiros restantes (Nikolas Ferreira, Eduardo Bolsonaro, Marco Feliciano, Damares Alves, Sergio Moro, General Mourão), todos cobertos por Capitão Braga no VIP.

`tsc --noEmit` validado: zero erros novos no arquivo alterado (confirmado isolando a saída por `grep -i "radar-politico"`, que não retornou nenhuma ocorrência). O único erro presente no projeto continua sendo o já documentado e pré-existente em `admin/usuarios/[id]/route.ts`, não relacionado a esta sessão.

Arquivos: `app/src/app/api/cron/radar-politico/route.ts` (corrigido).

**Deploy:** autorizado pelo usuário em 29/06/2026. Commit `606d479`, merge com 1 commit automático do bot `guardian-state` (sem conflitos), push para `origin/main` (`36f9ffa..5ff4917`). `vercel --prod` executado com sucesso (build 24s) — deploy `dpl_6V2nKPnbYkLqumbhoYdAY2d3RyHp` promovido a produção (`target: production`, `readyState: READY`), alias `alertapatriota.vercel.app` atualizado.

---

## FASE 35 — Backlog de segurança/infra adiado (Fases 21-23), retomado após a Fase 34

Usuário escolheu "Todos, por ordem de risco" para os 6 itens deferidos havia 3 fases (throttle entre crons WhatsApp, rate limit em `Map`, CSP, lockfile, heartbeat externo, prompt-injection RSS) — todos reconfirmados como ainda presentes no código antes de começar (grep/leitura direta, não memória de fases antigas).

**Item 1 — Throttle entre crons WhatsApp concorrentes: ✅ CONCLUÍDO**

Confirmado: ~19 rotas diferentes chamam `enviarMensagemGrupo`/`enviarMensagemPrivada` em `lib/whatsapp.ts` sem nenhuma coordenação entre si — se 2+ crons disparam quase ao mesmo tempo, o mesmo grupo do WhatsApp pode receber várias mensagens em rajada (risco de bloqueio anti-spam da Meta). Implementado claim atômico por plano: nova tabela `whatsapp_throttle (plano, ultimo_envio)`, e `enviarMensagemGrupo` agora chama `aguardarThrottleGrupo(plano)` antes de enviar — um `INSERT...ON CONFLICT DO UPDATE WHERE ultimo_envio <= NOW() - INTERVAL '3 seconds' RETURNING` que só deixa passar quem conseguir mover o timestamp pra agora; quem perde a corrida espera 750ms e tenta de novo (até 6 tentativas). Funciona entre invocações serverless distintas (ao contrário de um lock em memória). `enviarMensagemPrivada` (DM individual) não foi alterada — o risco real é rajada no mesmo grupo, não em destinatários diferentes.

Arquivos: `app/src/lib/whatsapp.ts`, `app/src/app/api/admin/setup/route.ts` (tabela nova), `app/src/app/api/cron/fiscal-codigo-schema/route.ts` (registrada em `SCHEMA_ESPERADO`, lição da Fase 30/Item 23).

**Item 2 — Rate limiting em `Map` na memória: ✅ CONCLUÍDO**

Confirmado: `criar-pix/route.ts` e `criar-direto/route.ts` ainda usavam `Map<string, number[]>` para limitar tentativas por IP — não sobrevive a ambiente serverless (cada cold start/instância concorrente tem memória isolada), o limite nunca era de fato global. Migrado para o mesmo padrão já usado em `leads/registrar/route.ts` desde a Fase 30 (Item 26): nova tabela `assinaturas_rate_limit (ip, rota, created_at)` compartilhada pelas duas rotas (coluna `rota` mantém o limite independente entre elas, mesmo comportamento de antes), limpeza oportunista de registros com mais de 1h a cada chamada (sem cron dedicado).

Arquivos: `app/src/app/api/assinaturas/criar-pix/route.ts`, `app/src/app/api/assinaturas/criar-direto/route.ts`, `app/src/app/api/admin/setup/route.ts` (tabela nova), `app/src/app/api/cron/fiscal-codigo-schema/route.ts` (registrada).

**Item 3 — Heartbeat sem monitoramento externo: ✅ CONCLUÍDO (ativado em 29/06/2026)**

Confirmado: `agente-heartbeat/route.ts` só alertava via Telegram — se o próprio GitHub Actions parasse de disparar este cron (já aconteceu, Fase 15), ninguém seria avisado, porque o aviso depende do cron rodar. Implementado `avisarDeadManSwitch()`: faz `fetch` pra `process.env.DEADMAN_SWITCH_URL` logo após o `enviarTelegram` de sucesso. Usuário criou conta grátis no healthchecks.io (check `alerta-patriota-paulo-ping`, schedule Simple, period 1 day, grace time 4h) e configurou a Ping URL. `DEADMAN_SWITCH_URL` adicionada como env var de produção na Vercel via CLI, seguida de `vercel --prod` (deploy `dpl_4DebMMR3VKC2hW6r8sVAF3j8awGX`) para a env var entrar em vigor. Ping testado manualmente em produção (`fetch` direto na Ping URL) — `200 OK`, recebido pelo healthchecks.io. A partir da próxima execução diária do cron (8h BRT) o ping passa a ser enviado automaticamente; se o cron não rodar dentro de 1 dia + 4h de margem, o healthchecks.io alerta por e-mail.

Arquivos: `app/src/app/api/cron/agente-heartbeat/route.ts`.

**Item 4 — Prompt-injection via conteúdo de RSS externo: ✅ CONCLUÍDO (mitigação, não blindagem completa)**

Confirmado: o filtro de idioma em `lib/ai.ts` (`REGEX_SCRIPT_NAO_PORTUGUES`) só bloqueia scripts não-latinos (CJK, cirílico, árabe etc.) — não pega instrução injetada em português/inglês dentro de um item de RSS, que é interpolado direto no prompt em ~10+ rotas (`resumir-noticias`, `gerar-card`, `bom-dia` etc.). Em vez de reescrever a interpolação em cada rota, a mitigação foi centralizada no único ponto por onde toda chamada já passa: `comInstrucaoIdioma()` (que já anexa a instrução de idioma a toda mensagem) agora também anexa `INSTRUCAO_ANTI_INJECAO`, deixando explícito ao modelo que conteúdo de notícia citado no prompt é dado de contexto, nunca uma instrução a seguir. Mitigação de baixo risco e abrangente, mas não é uma blindagem perfeita (modelos ainda podem ocasionalmente ser influenciados por injeção bem desenhada) — reduz a superfície real sem reescrever nenhuma rota individual.

Arquivos: `app/src/lib/ai.ts`.

**Item 5 — CSP ausente: ✅ CONCLUÍDO**

Confirmado via grep: o app não carrega nenhum `<script src>`/iframe externo (checkout do Mercado Pago é só PIX/PreApproval via API server-side, sem SDK/Brick no client); a única dependência externa real é a fonte do Google em `admin/layout.tsx` (`@import` direto de `fonts.googleapis.com`, não é o `next/font` embutido usado no resto do app). Adicionada `Content-Security-Policy` em `next.config.ts`: `default-src 'self'`, `script-src`/`style-src 'self' 'unsafe-inline'` (`'unsafe-inline'` necessário porque o projeto não usa nonce — Next.js injeta script de hydration inline e o admin usa `<style>` inline; ainda bloqueia qualquer `<script src>` de domínio externo, o vetor real de XSS/supply-chain que esta CSP existe pra fechar), `style-src`/`font-src` com exceção só para `fonts.googleapis.com`/`fonts.gstatic.com`, `img-src 'self' data:`, `connect-src 'self'`, `frame-ancestors 'none'`, `base-uri 'self'`, `form-action 'self'`. `npm run build` completo (não só `tsc --noEmit`) validado depois da mudança — build limpo, todas as rotas geradas normalmente.

Arquivos: `app/next.config.ts`.

**Item 6 — Lockfile ausente: ✅ CONCLUÍDO**

Confirmado: projeto não tinha `package-lock.json`/`pnpm-lock.yaml`/`yarn.lock` — builds não eram reprodutíveis (`npm install` podia trazer versão de dependência diferente a cada vez, inclusive entre o ambiente local e o build da Vercel). Gerado `package-lock.json` via `npm install --package-lock-only` (sem alterar `node_modules` já instalado). **Achado lateral, fora do escopo deste item:** `npm audit` aponta 4 vulnerabilidades moderadas, todas transitivas (`postcss` via Next.js, `uuid` via SDK do Mercado Pago) — corrigi-las exigiria downgrade de major version (`next@9`, `mercadopago@3` com breaking changes), não é uma correção de baixo risco; registrado aqui só como achado, não corrigido.

Arquivos: `app/package-lock.json` (novo).

`tsc --noEmit`: 0 erros novos em todos os 6 itens (único erro remanescente é o mesmo pré-existente e não-relacionado de `admin/usuarios/[id]/route.ts`). `npm run build` completo validado após o item 5 (CSP), cobrindo também os itens 1, 2, 4 e 6 já aplicados antes dele.

Arquivos totais desta fase: `app/src/lib/whatsapp.ts`, `app/src/lib/ai.ts`, `app/src/app/api/assinaturas/criar-pix/route.ts`, `app/src/app/api/assinaturas/criar-direto/route.ts`, `app/src/app/api/cron/agente-heartbeat/route.ts`, `app/src/app/api/admin/setup/route.ts`, `app/src/app/api/cron/fiscal-codigo-schema/route.ts`, `app/next.config.ts`, `app/package-lock.json` (novo).

**Deploy:** Commit `3b718db`, merge com 1 commit automático do bot `guardian-state` (sem conflitos), push para `origin/main` (`7890127..8c4c83d`). `vercel --prod` executado com sucesso (build 42s) — deploy `dpl_7H9Ko5v6oRYC5fV6YTuy7TgKAcc2` promovido a produção (`target: production`, `readyState: READY`), alias `alertapatriota.vercel.app` atualizado. `POST /api/admin/setup` chamado em produção imediatamente após o deploy (`{"ok":true,"mensagem":"Banco de dados criado com sucesso!"}`) e confirmado via query direta em `information_schema.tables` que `whatsapp_throttle` e `assinaturas_rate_limit` existem — fechando a janela de risco descrita abaixo antes de qualquer tráfego real depender delas. **Nota (risco já mitigado):** em `whatsapp.ts`, a query do throttle tem `.catch(() => [])` e simplesmente não aplica o atraso se a tabela não existir (degrada bem). Já em `criar-pix`/`criar-direto`, o `SELECT`/`INSERT` em `assinaturas_rate_limit` não tem `.catch()` — se a tabela não existisse, a exceção subiria e a rota responderia HTTP 500 ("Erro interno"), bloqueando o checkout. É o mesmo padrão (sem proteção) já usado em `leads/registrar` desde a Fase 30, não é um risco novo deste lote — mitigado nesta fase ao chamar o `admin/setup` em seguida do deploy, sem demora.

---

## PLANO MESTRE — AUDITORIA SEQUENCIAL POR PRIORIDADE (criado 30/06/2026)

**Status: 🔄 EM ANDAMENTO — Categoria 1 (Pagamentos/Assinaturas) em correção (Fase 41b).**

Pedido do usuário: criar um plano permanente para trabalhar as 7 categorias de auditoria (mesmas da Fase 30) em ordem de prioridade de negócio, **uma de cada vez**, sem pular para a próxima enquanto a atual não estiver com **zero problemas em aberto**. Prioridade definida pelo usuário em conversa (dinheiro real e risco legal vêm antes de tudo; o que é interno/recuperável vem por último).

### Regra obrigatória deste plano

> **Só se avança para a categoria N+1 quando a categoria N estiver com todos os achados 🔴/🟠/🟡 fechados** (corrigidos, validados e em produção). Achados ⚪ (informativos/nota positiva) não bloqueiam o avanço, só ficam registrados. Nenhuma categoria é trabalhada em paralelo com outra — é sequencial, não importa quanto tempo leve cada uma.

### Ordem de prioridade (1 = primeira a ser fechada, 7 = última)

| # | Categoria | Por que essa posição |
|---|-----------|------------------------|
| 1 | **Pagamentos/Assinaturas** | Dinheiro real do cliente — bug aqui é cobrança errada ou cliente pagando sem receber acesso. |
| 2 | **Infra/Segurança/LGPD** | Risco legal e de credenciais — vazamento ou descumprimento não tem como desfazer depois. |
| 3 | **WhatsApp/Mensagens** | É o canal de entrega do produto — bug aqui é vender e não entregar. |
| 4 | **Banco de Dados** | Sustenta as 3 categorias acima (constraint quebrada/N+1 pode derrubar pagamento ou mensagem). |
| 5 | **Agentes de Gestão/Crons Fiscais** | Rede de segurança que detecta e autocorrige as categorias acima — importante, mas é a 2ª linha, não a falha em si. |
| 6 | **Pipeline de Notícias** | Afeta qualidade/regularidade do conteúdo, mas é recuperável — não custa dinheiro nem expõe a empresa. |
| 7 | **Admin/Painel** | Uso interno (você e a equipe) — bug aqui não chega ao cliente final. |

### Como cada categoria será trabalhada (mesmo processo para as 7, sem exceção)

1. **Auditoria somente-leitura** da categoria inteira (subagentes paralelos quando fizer sentido, mesmo padrão das Fases 15/21/23/26/30) — nenhuma correção é feita nesta etapa.
2. **Lista de achados por severidade** (🔴 crítico, 🟠 alto, 🟡 médio, ⚪ informativo) apresentada a você antes de qualquer mudança de código.
3. **Correção item por item ou em lotes de baixo risco**, sempre seguindo o processo já estabelecido do projeto: avisar antes → implementar → validar (`tsc --noEmit` + `vitest run`) → registrar no `PLANO-CORRECAO.md` → avisar depois → pedir autorização explícita → commit/push/deploy.
4. **Re-checagem final da categoria**: confirmar que não sobrou nenhum 🔴/🟠/🟡 em aberto antes de declarar a categoria fechada.
5. Só então este plano avança para a próxima linha da tabela acima.
...
| 2. Infra/Segurança/LGPD | ✅ Concluída — Fase 46: 5ª re-auditoria + flag `Secure` no cookie de sessão do login |
| 3. WhatsApp/Mensagens | ✅ Concluída — Fases 47-53: 9 bugs corrigidos, 31 arquivos auditados, zero 🔴/🟠 em aberto |
| 4. Banco de Dados | 🔲 Aguardando autorização para iniciar — ⚠️ 1 achado pré-registrado (ver abaixo) |
...
**Próximo passo:** Categoria 3 concluída. Avançar para Categoria 4 — Banco de Dados. Auditoria somente-leitura → lista de achados → correção com autorização explícita.
...
| Data | Fase | Descrição |
|------|------|-----------|
| 02/07/2026 | Fase 54 | Varredura sistêmica transversal — Padrões 1 (maxDuration) e 2 (AbortSignal) em todos os 99 route.ts do projeto. Branch: `fix/maxduration-abortsignal-020726`. Metodologia: enumeração completa via PowerShell (`Get-ChildItem -Recurse -Filter "route.ts"` + `Select-String`) e varredura linha a linha de cada `fetch()` com janela de 15 linhas; falsos positivos confirmados por leitura integral do arquivo. **Resultados da varredura:** 99 route.ts totais; 44 com fetch(); 23 já tinham maxDuration; **21 sem maxDuration** identificados; **3 fetch() sem AbortSignal** confirmados em server-side (criar-pix → MP API, fiscal-especiais → dossie-elite interno, fiscal-trials → engajamento interno). **Fix P1 — maxDuration (21 arquivos):** `api/admin/agentes`, `api/assinaturas/criar-pix`, `api/cron/agente-heartbeat`, `api/cron/coletar-noticias`, `api/cron/coletar-noticias-global`, `api/cron/fiscal-api`, `api/cron/fiscal-codigo-logica`, `api/cron/fiscal-codigo-schema`, `api/cron/fiscal-especiais`, `api/cron/fiscal-facebook`, `api/cron/fiscal-fontes`, `api/cron/fiscal-login`, `api/cron/fiscal-trials`, `api/cron/fiscal-whatsapp`, `api/cron/gerente-codigo`, `api/cron/gerente-conteudo`, `api/cron/gerente-financeiro`, `api/cron/gerente-tecnico`, `api/cron/modo-crise`, `api/cron/revisor-logica`, `api/cron/revisor-schema`. Adicionado `export const maxDuration = 60` imediatamente antes de `export async function GET/POST`. **Fix P2-A (criar-pix):** fetch `https://api.mercadopago.com/v1/payments` sem `AbortSignal` — pagamento real em produção podia travar serverless indefinidamente até hard-kill da Vercel (502 sem log). Fix: `signal: AbortSignal.timeout(15000)` adicionado ao fetch do MP. **Fix P2-B (fiscal-especiais):** auto-fix interno para `dossie-elite` sem `AbortSignal` — se o dossiê elite estivesse lento, a chamada de auto-fix no sábado à noite travava até a Vercel matar. Fix: `signal: AbortSignal.timeout(30000)`. **Fix P2-C (fiscal-trials):** auto-fix interno para `engajamento` sem `AbortSignal` — mesma classe de bug, disparo de churn recovery podia travar. Fix: `signal: AbortSignal.timeout(30000)`. **Falsos positivos descartados:** `lista-de-espera/route.ts:67` (signal existe na linha 109, corpo HTML longo fez janela de 15 linhas não alcançar); `fiscal-pipeline/route.ts:120` (comentário `// fetch()`). **Não corrigidos nesta fase:** 16 `fetch()` em page.tsx client-side (browser, sem risco de serverless timeout — adiados por decisão do usuário). `tsc --noEmit`: 0 erros. `git diff --stat`: 21 arquivos, 45 linhas inseridas, 0 deletadas. Aguardando autorização para commit/push/deploy. |
| 02/07/2026 | Fase 53 | Encerramento da Categoria 3 (WhatsApp/Mensagens) — auditoria sistêmica de 31 arquivos (6 libs + 1 webhook + 1 admin route + 23 crons), 9 bugs encontrados e corrigidos. Todos 🟡 (timeout/timeout-evasion). **C3-1 (🟡) `cron/campanha-recuperacao/route.ts`:** sem `export const maxDuration = 60`. Sequência de até 8 contatos por cancelado (email + WhatsApp alternados em D1/D3/D7/D10/D15/D20/D25/D30) pode levar dezenas de segundos em volume. Fix: `export const maxDuration = 60`. **C3-2 (🟡) `cron/sequencia-nao-conversao/route.ts`:** sem `export const maxDuration = 60`. Loop sobre leads nas últimas 72h enviando até email + WhatsApp por lead; sem maxDuration a Vercel mata em 10s com vários leads pendentes. Fix: `export const maxDuration = 60`. **C3-3 (🟡) `cron/sequencia-nao-conversao/route.ts`:** função local `enviarEmail()` (linha 12) fazia `fetch` ao Brevo sem `AbortSignal.timeout` — duplicata local da lógica de `lib/brevo.ts`, não coberta pela correção sistêmica da Fase 52 (C2-5). Fix: `signal: AbortSignal.timeout(10000)` adicionado ao fetch da função local. **C3-4 (🟡) `cron/engajamento/route.ts`:** sem `export const maxDuration = 60`. 6 ondas de reengajamento (D5→D30) × até 40 usuários cada × email + WhatsApp = pode levar minutos. Fix: `export const maxDuration = 60`. **C3-5 (🟡) `cron/moderacao-grupo/route.ts`:** sem `export const maxDuration = 60`. `removerMembroGrupo()` chama `resolverJid()` + `chamarEvolution()` com 2 tentativas × 8s de AbortSignal + 1,5s de delay = até ~18s por membro; com mais de 1 cancelado a Vercel mata antes de terminar. Fix: `export const maxDuration = 60`. **C3-6 (🟡) `cron/publicar-noticias/route.ts`:** sem `export const maxDuration = 60`. `enviarMensagemGrupo()` → Evolution API; pico de latência excede 10s. Fix: `export const maxDuration = 60`. **C3-7 (🟡) `cron/upgrade-comportamental/route.ts`:** sem `export const maxDuration = 60`. Loop de até 5 usuários com `enviarMensagemPrivada()` + `setTimeout(2000)` por usuário = até 50s no pior caso. Fix: `export const maxDuration = 60`. **C3-8 (🟡) `cron/gerente-clientes/route.ts`:** sem `export const maxDuration = 60`. Path de escalonamento acumula `enviarTelegram()` (AbortSignal 5s) + `fetch` ao relatorio-ceo (AbortSignal 8s) = potencial >13s. Fix: `export const maxDuration = 60`. **C3-9 (🟡) `cron/relatorio-ceo/route.ts`:** sem `export const maxDuration = 60`. Tem `signal: AbortSignal.timeout(30000)` na chamada ao claude-resolver — esse AbortSignal sozinho excede o limite padrão de 10s da Vercel. Fix: `export const maxDuration = 60`. 22 arquivos auditados e confirmados LIMPOS (todos os libs, webhook, admin/mensagem, e 13 crons). `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. Aguardando autorização para commit/push/deploy. |
| 02/07/2026 | Fase 52 | Auditoria sistêmica de timeouts e XSS — 9 bugs encontrados e corrigidos em 9 arquivos (C2-1 a C2-9). Todos os achados são transversais (infra/segurança) mas distribuídos em múltiplas categorias do plano. **C2-1 (🟡) `api/lista-de-espera/route.ts`:** sem `export const maxDuration`. Rota pública que faz SELECT+INSERT no banco (rate limit), INSERT em `lista_espera`, self-fetch para Brevo e INSERT em `agentes_log` — pior caso ~3s de I/O sequencial + e-mail. Sem maxDuration, pico de latência do Neon ou do Brevo mata a função em 10s da Vercel antes da resposta ao usuário. Fix: `export const maxDuration = 60`. Fix adicional: fetch direto ao Brevo sem `AbortSignal.timeout` — adicionado `signal: AbortSignal.timeout(10000)`. **C2-2 (🟡) `api/cron/guardiao-seguranca/route.ts`:** sem `export const maxDuration`. Loop dispara `dispararAgente()` com `AbortSignal.timeout(8000)` para 5 agentes do SQUAD_REVISAO sequencialmente — pior caso 40s. Vercel matava antes do 2º agente sem maxDuration. Fix: `export const maxDuration = 60`. **C2-3 (🟡) `api/cron/agente-limpeza/route.ts`:** sem `export const maxDuration`. Rota apaga registros antigos de várias tabelas e envia Telegram — múltiplas queries + I/O externo. Fix: `export const maxDuration = 60`. **C2-4 (🟡) `api/cron/backup/route.ts`:** sem `export const maxDuration` (parcialmente — `limparBackupsAntigos` já tinha AbortSignal.timeout internamente). A branch de backup do dia era criada via `fetch` POST à Neon API sem `AbortSignal.timeout`. Fix: `export const maxDuration = 60` + `signal: AbortSignal.timeout(15000)` no fetch de criação de branch. **C2-5 (🟡) `lib/brevo.ts` (sistêmico):** `enviarEmail()` (função base chamada por 5 funções de e-mail) fazia `fetch` à API do Brevo sem `AbortSignal.timeout`. Qualquer lentidão do Brevo travava a função serverless chamadora indefinidamente até o kill da Vercel. Fix: `signal: AbortSignal.timeout(10000)` no fetch interno de `enviarEmail()` — cobre automaticamente `enviarEmailBoasVindas`, `enviarEmailCancelamento`, `enviarEmailInadimplente`, `enviarEmailRecuperacao`, `enviarEmailReengajamento`. **C2-6 (🟡) `lib/facebook.ts` (sistêmico, 5 fetches):** todas as 5 funções (`publicarPostFacebook`, `buscarComentariosNaoRespondidos` ×2, `responderComentario`, `verificarTokenFacebook`) faziam fetch à Graph API sem `AbortSignal.timeout`. Fix: `signal: AbortSignal.timeout(15000)` em todos os 5 call sites. **C2-7 (🟡) `lib/instagram.ts` (sistêmico, 11 fetches):** todas as chamadas à Graph API / aguardar processamento de vídeo sem AbortSignal. Fix: `signal: AbortSignal.timeout(30000)` nos dois container-upload (vídeo pode demorar), `AbortSignal.timeout(15000)` nos demais 9 fetches (`media_publish`, `buscarComentariosIG` ×2, `responderComentarioIG`, `enviarDMInstagram`, `atualizarBioLink`, `verificarTokenIG`, polling interno de `aguardarProcessamento`). **C2-8 (🟡) `api/admin/modo-crise/route.ts`:** sem `export const maxDuration` + self-fetch ao cron `modo-crise` sem `AbortSignal.timeout`. Fix: `export const maxDuration = 60` + `signal: AbortSignal.timeout(15000)` no self-fetch. **C2-9 (🟠) `app/admin/mensagens/page.tsx`:** componente `PreviewWPP` aplicava regex de bold/italic diretamente sobre o input do usuário e passava para `dangerouslySetInnerHTML` sem escapar HTML primeiro. Um admin que digitasse `<script>alert(1)</script>` ou `<img onerror=...>` via textarea causaria XSS no próprio painel. Fix: escaping de 4 entidades HTML (`&`→`&amp;`, `<`→`&lt;`, `>`→`&gt;`, `"`→`&quot;`) antes das regex, depois os padrões `*bold*` e `_itálico_` ainda funcionam normalmente sobre o texto já seguro. Impacto limitado (área admin autenticada), mas padrão correto independente do escopo. `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. Aguardando autorização para commit/push/deploy. |
| 02/07/2026 | Fase 51 | Redo completo da Categoria 1 (Pagamentos/Financeiro) com enumeração sistemática — 22 arquivos lidos sem exceção. 3 bugs encontrados e corrigidos. **Bug 1 (🔴) `cron/fiscal-apis-externas/route.ts`:** `testarApi()` criava `AbortController` mas nunca passava `controller.signal` para `fn()` — as 4 chamadas fetch externas (Cloudinary, Evolution, Mercado Pago, Brevo) rodavam sem nenhuma proteção de abort. Chamada hanging travava o `Promise.all` até a Vercel matar a função em 10s no hard-kill — sem log, sem alerta, sem resposta. O sistema de monitoramento de APIs ficava cego exatamente quando mais precisava funcionar. Fix: assinatura de `fn` alterada para `(signal: AbortSignal) => Promise<Response>`, `AbortSignal.timeout(TIMEOUT_MS)` passado diretamente para cada `fn(signal)`, AbortController/setTimeout removidos. Todos os 4 call sites atualizados para `(signal) => fetch(..., { ..., signal })`. `export const maxDuration = 60` adicionado (sem ele, o Promise.all com 5 checks simultâneos de até 8s poderia extrapolar os 10s padrão da Vercel). **Bug 2 (⚪) `cron/fiscal-codigo-seguranca/route.ts`:** sem `export const maxDuration`. Rota executa 5 checks sequenciais com `AbortSignal.timeout(8000-10000)` cada — pior caso ~50s. Sem maxDuration, Vercel mata em 10s após o 1º check que travar; os checks restantes nunca rodam. Fix: `export const maxDuration = 60`. **Bug 3 (⚪) `cron/reconciliador-pix/route.ts`:** sem `export const maxDuration`. Loop de até 50 pagamentos PIX, cada um fazendo `paymentClient.get()` no SDK do MP sequencialmente — sem timeout individual. Dia com MP lento ou 30+ PIXes pendentes → função morta pela Vercel em 10s no meio da reconciliação. Fix: `export const maxDuration = 60`. **Pendente da Categoria 3 (gerar-card):** fetch de `sendMedia` sem `AbortSignal.timeout` — corrigido junto nesta fase: `signal: AbortSignal.timeout(30000)` adicionado ao fetch inline da Evolution API em `cron/gerar-card/route.ts:111`. `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. |
| 01/07/2026 | Fase 50 | Arquivo `admin/publicar-agora/route.ts` — não coberto nos 4 passes anteriores da Categoria 3. 1 achado 🟡 e 1 achado ⚪ corrigidos. **Fix 1 (🟡):** sem `export const maxDuration`. A rota faz 2 self-fetch sequenciais com `AbortSignal.timeout(15000)` cada; sem maxDuration a Vercel mata a função em 10s antes do AbortSignal disparar → admin recebe 502 antes de completar a primeira publicação. Fix: `export const maxDuration = 60`. **Fix 2 (⚪):** `&noticia_id=${noticia_id}` sem `encodeURIComponent` na URL da self-invocação. Um noticia_id com caracteres especiais poderia injetar parâmetros extras na chamada interna. Fix: `encodeURIComponent(noticia_id)`. `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. |
| 01/07/2026 | Fase 49 | 4ª varredura da Categoria 3 (WhatsApp/Mensagens) — lidos os 2 arquivos finais não cobertos nas rodadas anteriores: `admin/usuarios/[id]/route.ts` e `admin/setup/route.ts`. 1 achado 🟡 e 3 achados ⚪ corrigidos. **Fix 1 (🟡):** `admin/usuarios/[id]/route.ts` — sem `export const maxDuration`. Ações `cancelar` e `excluir_dados` fazem MP PreApproval + `resolverJid` (AbortSignal 8s) + `chamarEvolution` (AbortSignal 8s × 2 tentativas + 1,5s entre elas) — pior caso ~20s+. Vercel mata em 10s → admin recebe 502. Fix: `export const maxDuration = 60`. **Fix 2 (⚪):** `admin/usuarios/[id]/route.ts` ação `cancelar` — `UPDATE membros_grupos` e `UPDATE grupos_whatsapp SET membros_ativos -= 1` aconteciam fora do `sql.transaction([...])`. Se a transação falhasse, o membro ficava marcado como `'removido'` nos grupos mas `usuarios.status` continuava `'ativo'`. Fix: grupoId coletado antes e queries de grupo incluídas no mesmo `sql.transaction` via array dinâmico. **Fix 3 (⚪):** `admin/usuarios/[id]/route.ts` ação `excluir_dados` — `UPDATE usuarios` (anonimização) e `UPDATE assinaturas SET cancelada` eram dois `await` separados. Se o segundo falhasse após o primeiro commitado, usuário ficaria anonimizado mas assinatura permanece `'ativa'`, bloqueando re-assinatura via `idx_assinaturas_usuario_ativa`. Fix: `sql.transaction([...])` envolvendo os dois. **Fix 4 (⚪):** `lib/brevo.ts` função `enviarEmailRecuperacao` — subject usava `${nome}` cru enquanto todos os HTML bodies já usavam `${nomeSeguro}` desde a Fase 48. Um `nome` com `\r\n` poderia injetar cabeçalhos (risco prático baixo via JSON→HTTPS→Brevo, mas inconsistente com o padrão defensivo). Fix: `nome.replace(/[\r\n]/g, '')` no subject (HTML-escape não se aplica a subjects — exibiria `&amp;` literalmente). `admin/setup/route.ts`: LIMPO — apenas DDL, sem input de usuário interpolado. `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. |
| 01/07/2026 | Fase 48 | 3ª varredura da Categoria 3 (WhatsApp/Mensagens) — lidos 9 arquivos adicionais (radar-politico, radar-economico, termometro, personagem-semana, lib/ai.ts, lib/alertas.ts, lib/telegram.ts, admin/mensagem, lib/personas.ts). 4 achados ⚪ corrigidos. **Fix 1:** `lib/whatsapp.ts` — `chamarEvolution()` e `resolverJid()` sem `AbortSignal.timeout`; Evolution API hanging bloqueava a função serverless até Vercel matar. Fix: `{ ...options, signal: AbortSignal.timeout(8000) }` em ambos. **Fix 2:** `lib/telegram.ts` — `enviarTelegram()` sem `AbortSignal.timeout`; Telegram API hanging bloqueava rotas que não usam `.catch()` (radar-politico, termometro). Fix: `signal: AbortSignal.timeout(5000)`. **Fix 3:** `cron/bot-responder/route.ts` — mensagens de membros do WhatsApp interpoladas com aspas duplas no prompt Claude, permitindo prompt injection. Fix: delimitadores XML `<mensagem>...</mensagem>` em ambos os prompts (Braga e Cavalcanti), padrão recomendado pela Anthropic para isolar entrada não-confiável. **Fix 4:** `lib/brevo.ts` — `nome` (user-supplied) interpolado diretamente em HTML de e-mail sem escape. Fix: adicionada função `escaparHtml()` e aplicada em `nomeSeguro` em todas as funções de e-mail; `firstName` em `enviarEmailReengajamento` também escapa antes de entrar nos templates. `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. |
| 01/07/2026 | Fase 47 | 2ª varredura da Categoria 3 (WhatsApp/Mensagens) — lidos arquivos não cobertos na 1ª rodada: `lib/brevo.ts`, `cron/cacador-desistentes`, `cron/agente-medico`, `cron/preditor-churn`. 1 achado 🟡 encontrado e corrigido. **Fix (🟡):** `cron/preditor-churn/route.ts` — sem `export const maxDuration`. O Fase 30 adicionou `await new Promise(r => setTimeout(r, 2000))` entre cada mensagem privada (proteção anti-ban Evolution API), mas não adicionou `maxDuration = 60`. Com apenas 5 usuários de alto risco, o loop leva 10+ segundos — a Vercel mata a função no timeout padrão de 10s. O agente Rodrigo Risco terminava incompleto em toda execução com mais de 4-5 usuários no threshold 70+. Fix: adicionado `export const maxDuration = 60` com comentário explicativo, mesmo padrão do `agente-medico/route.ts`. `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. |
| 01/07/2026 | Fase 46 | 5ª re-auditoria da Categoria 2 (Infra/Segurança/LGPD) — 24 arquivos lidos, zero 🔴/🟠. Um achado 🟡 corrigido. **Fix (🟡):** `app/login/page.tsx:39` — Server Action `fazerLogin` setava cookie de sessão via `cookieStore.set()` sem `secure: true`. O helper `setCookieToken()` em `lib/auth.ts` adiciona `; Secure` em produção corretamente, mas o Server Action bypassa o helper. Sem a flag `Secure`, o browser pode enviar o cookie sobre HTTP antes de cachear o HSTS (primeiro acesso). Fix: adicionado `secure: process.env.NODE_ENV === "production"`. `vitest run`: 63/63 passando. `tsc --noEmit`: 0 erros. |
| 01/07/2026 | Fase 45 | 4ª re-auditoria da Categoria 2 (Infra/Segurança/LGPD) — zero 🔴/🟠. Um achado 🟡 corrigido antes de avançar. **Fix (🟡):** `api/webhook/whatsapp/route.ts` — comparação do `EVOLUTION_WEBHOOK_SECRET` usava `===` (timing-unsafe), inconsistente com o padrão `timingSafeEqual` já estabelecido em `verificarCronSecret()`. Fix: adicionado `import { timingSafeEqual } from "crypto"` e substituída a comparação por closure `comparar()` com `timingSafeEqual`. Endpoint público; exploitabilidade prática baixa, mas fix corrige inconsistência com o padrão de segurança do projeto. `vitest run`: 63/63 passando. `tsc --noEmit`: 0 erros. Categoria 2 encerrada. |
| 01/07/2026 | Fase 44 | 3ª re-auditoria completa da Categoria 2 (Infra/Segurança/LGPD) — 33 arquivos lidos, zero 🔴/🟠. Dois achados menores corrigidos antes de avançar para Categoria 3 (usuário escolheu Opção A). **Fix 1 (🟡):** `api/admin/noticias/route.ts` e `api/admin/posts-whatsapp/route.ts` — `limite` sem `Math.min()`. Mesmo padrão já corrigido em `admin/logs` e `admin/usuarios` nas Fases 42/43. Fix: `Math.min(parseInt(...), 200)` em ambos. **Fix 2 (⚪):** `api/cron/fiscal-login/route.ts` — testava `/api/auth/login` removida na Fase 27. Rota inexistente retornava 404, nunca 500, então o check nunca disparava alerta; o teste era código morto. Fix: bloco `fetch` de `/api/auth/login` removido; comentário de cabeçalho desatualizado limpo. Categoria 2 encerrada sem achados 🔴/🟠/🟡 em aberto. `vitest run`: 63/63 passando. |
| 01/07/2026 | Fase 43 | Re-auditoria sem viés da Categoria 2 (Infra/Segurança/LGPD) — 2 novos bugs corrigidos. **Bug 1 (🟡):** `api/lista-de-espera/route.ts` — rota pública sem rate limit. Cada chamada inseria linha no banco, logava em `agentes_log` e disparava e-mail via Brevo. Spam poderia esgotar a cota diária do Brevo impedindo e-mails de boas-vindas/assinatura de usuários reais. Fix: adicionado `excedeuLimite()` com o mesmo padrão DB-backed das demais rotas públicas (5 req / 10 min por IP via `assinaturas_rate_limit WITH rota='lista-espera'`). **Bug 2 (🟡):** `api/admin/logs/route.ts` — `limite` sem teto (`parseInt(...)`). Mesmo padrão já corrigido em `admin/usuarios` na Fase 42. Fix: `Math.min(parseInt(...), 500)`. Achados informativos sem ação: timing oracle mínimo no login (sem bcrypt para e-mail não encontrado — impraticável com rate limit 5/10min + único admin); `ignoreBuildErrors` deferred para Category 4; `EVOLUTION_WEBHOOK_SECRET` rotation operacional. `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. Deploy `dpl_9SsDM5X5MPoBUCSJBUpfRbyo5BY7` produção. |
| 01/07/2026 | Fase 42 | Auditoria da Categoria 2 (Infra/Segurança/LGPD) — 2 bugs corrigidos. **Bug 1 (🟠):** `login/page.tsx` — Server Action `fazerLogin` sem rate limit e sem logging de tentativas falhas. Atacante com e-mail do admin podia testar senhas indefinidamente sem ser detectado nem bloqueado; o `guardiao-seguranca` monitorava `agentes_log WHERE agente LIKE '%login%'` mas a action nunca escrevia nada lá. Fix: adicionado rate limit DB-backed (5 tentativas / 10 min por IP via `assinaturas_rate_limit WITH rota='login'`) e logging de falhas em `agentes_log` (agente `lisa-login`, motivos: `email_nao_encontrado`, `senha_incorreta`, `nao_e_admin`). Rate limit falha silenciosamente (mesmo comportamento de credencial errada) sem revelar ao atacante que foi bloqueado. **Bug 2 (🟡):** `admin/usuarios/route.ts GET` — parâmetro `limite` sem teto (`parseInt(...)`). Admin autenticado podia pedir `?limite=999999` e dumpar todos os usuários numa query. Fix: `Math.min(limite, 500)`. **Bug de documentação (🟠 deferred):** `next.config.ts` com `typescript: { ignoreBuildErrors: true }` — bloqueia detecção de erros de tipo no build. Depende da Category 4 (lib/db.ts tipos). Não corrigido agora, será removido quando Category 4 corrigir os tipos. Área limpa auditada (sem bugs): JWT+timingSafeEqual, cookies HttpOnly/Secure, CSP completa, HSTS 2 anos, webhook HMAC MP e Evolution, todas as rotas admin com requireAdmin(), Claude Resolver com ARQUIVOS_PROTEGIDOS, rate limit DB-backed em 3 rotas públicas, proteção CSV injection, LGPD aceite + retenção 180 dias. `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. Deploy `dpl_DD31mvS3j1xH5hsKixkFZQuHXyLr` produção. |
| 30/06/2026 | Fase 41b | Auditoria completa da Categoria 1 revelou que o Bug 3 da Fase 41 estava incompleto: `criar-pix`/`criar-direto` passaram a permitir o início de nova assinatura para PIX anual expirado (>360 dias), mas `ativarAcesso` ainda disparava 23505 (`idx_assinaturas_usuario_ativa`) quando o webhook de aprovação chegava — a assinatura PIX antiga continuava com `status='ativa'` no banco, bloqueando o INSERT da nova. Resultado: o usuário pagava o novo PIX mas recebia o alerta 🔴 "estorno manual necessário" sem nunca ter o acesso renovado. Fix: adicionado `UPDATE assinaturas SET status='cancelada' WHERE usuario_id=... AND status='ativa' AND ciclo='anual' AND renovada_em IS NULL AND created_at < NOW() - INTERVAL '360 days'` em `lib/mp-ativar-acesso.ts` **antes** de `sql.transaction()`, com `.catch(() => {})` para não bloquear o caminho normal (se a limpeza falhar, o 23505 da transaction ainda dispara o alerta padrão como fallback). Novo teste adicionado em `mp-ativar-acesso.test.ts` verificando que a UPDATE de limpeza é chamada. Resto da auditoria confirmou zero achados novos: webhook HMAC, mrr, cupons, fiscal-pagamentos, admin/financeiro, admin/financeiro/pagamentos e pagamento/sucesso todos corretos. Achados informativos (sem correção): dedup do webhook não cobre renewals/cancellations (operações SQL são idempotentes, só emails/alertas duplicados em retry do MP — baixo impacto); fiscal-pagamentos usa usuarios.status como proxy (heurística suficiente). `tsc --noEmit`: 0 erros. `vitest run`: 63/63 passando. Aguardando autorização para commit/push/deploy. |
| 30/06/2026 | Fase 41 | Auditoria aprofundada de lógica e função da Categoria 1 (Pagamentos/Assinaturas), autorizada pelo usuário ("faça uma auditoria detalhada e minuciosa [...] sem deixar passar nada"). 3 achados 🟠 encontrados e apresentados antes de qualquer alteração de código. **Bug 1 (🟠):** `renovarAcesso()` em `lib/mp-desativar-acesso.ts` — a query `UPDATE assinaturas` atualizava apenas `renovada_em`, nunca restaurava `status = 'ativa'`. Resultado: usuário que passou pelo ciclo inadimplente→renovação ficava com `assinaturas.status='inadimplente'` para sempre, mesmo continuando a pagar. `calcularMRR()` filtra `WHERE status='ativa'`, logo esses assinantes saíam do MRR. Fix: adicionado `status = 'ativa'` ao UPDATE. Teste atualizado para verificar que a query contém tanto `status = 'ativa'` quanto `renovada_em`. **Bug 2 (🟠):** `admin/usuarios/[id] reativar` + `criar-direto`/`criar-pix` — a ação `reativar` seta `usuarios.status='ativo'` sem recriar a assinatura no MP. As rotas de assinatura checavam `usuarios.status === 'ativo'` como proxy de "tem assinatura ativa", bloqueando com 409 o usuário manualmente reativado de completar a nova cobrança. Fix: substituído o check de `usuarios.status` por uma query em `assinaturas WHERE status='ativa'` nos dois flows. **Bug 3 (🟠):** PIX anual sem renovação — `assinaturas` de PIX nunca expiram (não há webhook de renovação para PIX, diferente do cartão). Após 360 dias, o assinante ficava bloqueado de fazer um novo PIX porque a assinatura antiga constava como 'ativa'. Fix (integrado ao fix do Bug 2): a query de bloqueio exclui assinaturas com `ciclo='anual' AND renovada_em IS NULL AND created_at < NOW() - INTERVAL '360 days'` (expired PIX annual), permitindo a renovação sem precisar de um cron de expiração separado. Arquivos alterados: `lib/mp-desativar-acesso.ts`, `tests/lib/mp-desativar-acesso.test.ts`, `assinaturas/criar-pix/route.ts`, `assinaturas/criar-direto/route.ts`. `tsc --noEmit`: 0 erros. `vitest run`: 62/62 passando. Aguardando autorização para commit/push/deploy. |
| 30/06/2026 | Fase 40b | Re-auditoria completa e linha-por-linha da Categoria 1 (Pagamentos/Assinaturas) pedida pelo usuário após fechamento da Fase 40. Cobertos pela primeira vez nesta rodada: `lib/brevo.ts`, `lib/db.ts`, `admin/setup/route.ts`, `admin/usuarios/[id]/route.ts` (ação cancelar/reativar/excluir_dados), `tests/lib/mp-desativar-acesso.test.ts`, `tests/lib/mp-webhook-hmac.test.ts`. 1 achado 🟡 novo encontrado e apresentado antes de qualquer código ser alterado: **`admin/usuarios/[id]/route.ts` — ação `cancelar` fazia 2 UPDATE SQL separados e não-atômicos** (usuarios + assinaturas). Se o 1º sucedia e o 2º falhava: usuario ficava 'cancelado' mas assinatura permanecia 'ativa' — inflava MRR e bloqueava re-assinatura com 23505 no idx_assinaturas_usuario_ativa. O caminho do webhook (`mp-desativar-acesso.ts:50`) já usava `sql.transaction([...])` corretamente; o admin não. Corrigido com `sql.transaction([...])` no mesmo padrão. Achados informativos (sem impacto): tipo TypeScript de `Assinatura`/`Pagamento` em `lib/db.ts` diverge do schema real (colunas adicionadas por ALTER TABLE não refletidas nos tipos); `lib/brevo.ts` interpola `nome` diretamente em HTML de e-mail transacional — afeta só o próprio usuário. Nenhum 🔴/🟠 encontrado em nenhum dos 17 arquivos lidos. `tsc --noEmit`: 0 erros. `vitest run`: 62/62 passando. Aguardando autorização para commit/push/deploy. |
| 30/06/2026 | Fase 40 | Auditoria somente-leitura completa da Categoria 1 (Pagamentos/Assinaturas) — primeiro passo do Plano Mestre, autorizado pelo usuário. Cobertos: `webhook/mercadopago/route.ts`, `lib/mp-ativar-acesso.ts`, `lib/mp-desativar-acesso.ts`, `lib/mp-webhook-hmac.ts`, `assinaturas/criar-pix`, `assinaturas/criar-direto`, `lib/cupons.ts`, `lib/mrr.ts`, `cron/reconciliador-pix`, `cron/fiscal-pagamentos`, `admin/financeiro` (+`/pagamentos`), `pagamento/sucesso/page.tsx`. 1 achado 🟡 encontrado e apresentado ao usuário antes de qualquer código ser alterado: **`reconciliador-pix` reportava 🟢 "PIX reconciliado" mesmo quando `ativarAcesso()` internamente abortava por assinatura duplicada (23505)** — resultado: dois alertas contraditórios (🔴 "estorno manual" + 🟢 "reconciliado") chegavam em sequência, confundindo o cenário que mais exige clareza de alerta. Usuário autorizou a correção ("sim"). Fix aplicado: `ativarAcesso()` (lib/mp-ativar-acesso.ts) passou a retornar `Promise<boolean>` — `true` se o acesso foi ativado, `false` no caminho de duplicata (23505); `reconciliador-pix/route.ts` usa o retorno para decidir: `true` → incrementa `reconciliados` e envia o 🟢; `false` → incrementa novo contador `duplicados` (sem 🟢 extra, pois o 🔴 interno já foi enviado com contexto suficiente). Testes de `mp-ativar-acesso.test.ts` atualizados para validar o novo contrato de retorno (caminho feliz: `true`; duplicata 23505: `false`). `tsc --noEmit`: 0 erros. `vitest run`: 62/62 passando. Sem commit/push/deploy — aguardando autorização do usuário. |
| 30/06/2026 | Plano Mestre | Usuário pediu um plano permanente para fechar as 7 categorias de auditoria (mesmas da Fase 30) em ordem de prioridade de negócio, uma de cada vez, só avançando para a próxima quando a atual estiver com zero achados 🔴/🟠/🟡 em aberto. Ordem definida em conversa com o usuário: 1) Pagamentos/Assinaturas, 2) Infra/Segurança/LGPD, 3) WhatsApp/Mensagens, 4) Banco de Dados, 5) Agentes de Gestão/Crons Fiscais, 6) Pipeline de Notícias, 7) Admin/Painel. Plano registrado na nova seção "PLANO MESTRE — AUDITORIA SEQUENCIAL POR PRIORIDADE" (antes de "CREDENCIAIS E REFERÊNCIAS"), com o processo padrão por categoria (auditoria somente-leitura → lista de achados por severidade → correção item por item com autorização explícita → validação → registro → deploy → re-checagem final) e tabela de andamento. Nenhuma auditoria foi iniciada ainda — aguardando ordem explícita do usuário para começar pela Categoria 1. |
| 30/06/2026 | Fase 39b | Item 2 da Fase 39 (idempotência instável do PIX), deixado pendente naquela fase, corrigido agora com autorização do usuário ("pode tratar agora"). Em `criar-pix/route.ts`, `idempotencyKey = randomUUID()` gerada a cada requisição HTTP virou `createHash("sha256").update(\`${usuarioId}|${plano}|criar-pix|${bucket}\`)` com `bucket = Math.floor(Date.now() / 10min)` — retry de rede ou duplo-clique no botão "Gerar PIX" dentro da mesma janela de 10min agora reaproveita a mesma `Idempotency-Key`, então o Mercado Pago devolve a cobrança já criada em vez de gerar um PIX pendente novo a cada tentativa (evita poluir `pagamentos` com pendentes órfãos que confundiam o reconciliador da Fase 33 Item 1). Geração da chave movida para depois da resolução de `usuarioId` (precisa dele) e do `validarCupom`, mesmo ponto onde `valor` final já está calculado. `tsc --noEmit`: 0 erros. `vitest run`: 62/62 passando, sem regressão. **Deploy:** usuário autorizou ("sim"). Commit `c38f3b5` (sem commits novos do bot no remoto, push direto, `1b490af..c38f3b5`). `vercel --prod` executado com sucesso — deploy `dpl_3FtsoLDnGqVTBXqB5F2jKUdvnAKg` promovido a produção, alias `alertapatriota.vercel.app` atualizado. |
| 30/06/2026 | Fase 39 | Re-auditoria pontual do item "🟡 2 achados adicionais de Pagamentos" da Fase 30, categoria 1 — citado só como contagem desde então (Fase 33 já registrava como pendente de re-checagem), nunca detalhado no documento. Lidas as 3 rotas `assinaturas/*` (`criar`, `criar-pix`, `criar-direto`), `webhook/mercadopago/route.ts` e `lib/cupons.ts`, cruzando com o que o frontend (`page.tsx`) e os crons fiscais realmente chamam. 2 achados identificados e apresentados ao usuário antes de qualquer correção: (1) `assinaturas/criar/route.ts` (fluxo de assinatura via cartão para usuário já autenticado, criada antes de `criar-direto` existir) é **rota órfã** — nenhum fluxo real do produto a chama (`page.tsx` usa só `criar-direto`/`criar-pix`); o único chamador era o próprio cron `fiscal-codigo-seguranca` testando que ela rejeita sem auth. Mesmo órfã, ficava live em produção, autenticada, criando assinaturas reais no Mercado Pago, sem o rate limiting DB-backed que os irmãos `criar-pix`/`criar-direto` ganharam na Fase 34/35. (2) Em `criar-pix/route.ts:69`, a `Idempotency-Key` enviada ao Mercado Pago (`randomUUID()`) é gerada a cada requisição HTTP, não por tentativa de compra — retry de rede ou duplo-clique no botão "Gerar PIX" gera uma chave nova a cada vez, então a API do MP não deduplica; não causa cobrança duplicada (PIX só cobra se pago), mas pode poluir `pagamentos` com pendentes órfãos. Usuário perguntou se a rota `criar` deveria ser removida; recomendado que sim (mesmo precedente já usado na Fase 27 item 1 para `auth/cadastro`, outra rota morta excluída) — usuário autorizou. Aplicado apenas o item 1 nesta fase (remoção): deletado `app/src/app/api/assinaturas/criar/route.ts`; em `app/src/app/api/cron/fiscal-codigo-seguranca/route.ts`, removido o check `assinatura_sem_auth` (testava a rota agora inexistente), substituído por comentário no mesmo padrão já usado para a remoção de `auth/cadastro` na Fase 27. Item 2 (idempotência instável do PIX) **ainda não corrigido** — fica para uma próxima fase, mediante autorização do usuário. `rm -rf .next` necessário antes do `tsc --noEmit` (cache antigo apontava para o módulo deletado e mascarava o erro pré-existente de `admin/usuarios/[id]/route.ts`, que não reapareceu depois — possível falso positivo do cache, não investigado a fundo por ser inofensivo). `tsc --noEmit`: 0 erros. `vitest run`: 62/62 passando, 13 arquivos. **Deploy:** usuário autorizou ("sim"). Commit `7268510` (sem commits novos do bot no remoto, push direto). `vercel --prod` executado com sucesso — deploy `dpl_G4Tv2xYdXKnLJQ2uRocgZa45rvGZ` promovido a produção, alias `alertapatriota.vercel.app` atualizado. |
| 30/06/2026 | Fase 38 | Continuação direta da Fase 37: usuário pediu revisão de tudo antes do deploy ("revise os arquivos primeiro, corrija os erros e depois sim commit, push e deploy"). Revisão linha a linha de todos os 8 arquivos `lib/` cobertos contra os testes correspondentes não encontrou erros novos (só os 2 já autocorrigidos durante a escrita). `npm run lint`/`npx eslint` falham por config pré-existente e não-relacionada (`eslint.config.js` errado sendo resolvido a partir da pasta raiz "BioNexus Digital", faltando pacote `@eslint/js`) — fora de escopo, não corrigido. Perguntado ao usuário se a cobertura existente era suficiente ou se faltava algo importante; identificado que o código de maior risco financeiro (webhook do Mercado Pago — ativação/renovação/desativação de acesso pago, validação HMAC) não tinha nenhum teste, ao contrário dos demais `lib/`. Usuário autorizou explicitamente ("Sim, incluir agora") extrair esse código para testá-lo antes do deploy. Criados 2 novos arquivos `lib/`: `lib/mp-desativar-acesso.ts` (funções `renovarAcesso`/`desativarAcesso`, extraídas verbatim de `webhook/mercadopago/route.ts`, mesmo padrão da extração de `ativarAcesso` na Fase 33) e `lib/mp-webhook-hmac.ts` (função pura `validarAssinaturaWebhook()`, sem `console.log`, extraída da lógica de validação HMAC do webhook — mesmo comportamento: sem `x-signature` aceita, com `x-signature` mas sem secret configurada rejeita, tenta os 3 formatos de manifest do padrão já documentado). `webhook/mercadopago/route.ts` atualizado para importar as duas funções em vez de manter definições locais duplicadas; `validarWebhook()` virou wrapper fino que chama `validarAssinaturaWebhook()` e preserva os mesmos logs de diagnóstico. 3 novos arquivos de teste, 15 testes: `tests/lib/mp-ativar-acesso.test.ts` (4: caminho feliz com grupo+mensagem+e-mail, sem telefone cadastrado, falha ao adicionar no grupo, duplicidade de assinatura 23505 com retorno antecipado), `tests/lib/mp-desativar-acesso.test.ts` (4: usuário não encontrado, cancelamento bem-sucedido com remoção do grupo, falha na remoção do grupo loga erro em vez de marcar 'removido', `renovarAcesso` monta lote atômico de 3 escritas), `tests/lib/mp-webhook-hmac.test.ts` (7: sem x-signature aceita, x-signature sem secret rejeita, assinatura malformada rejeita, formato completo/sem request-id/minimalista todos aceitos calculando o HMAC esperado de forma independente via `node:crypto`, assinatura que não bate em nenhum formato rejeita). `npm test`: 62/62 passando, 13 arquivos. `tsc --noEmit`: 0 erros novos (único remanescente, igual às fases anteriores, é o mesmo pré-existente de `admin/usuarios/[id]/route.ts`). **Deploy:** usuário autorizou ("sim") commit/push/deploy do lote Fase 37+38. Commit `a18858e`, merge com bot `guardian-state` + push (`6c2f39b..6f4d645`). `vercel --prod` executado com sucesso (build 52s) — deploy `dpl_2tynWefikyjEqN1g6U6A47g18L3S` promovido a produção, alias `alertapatriota.vercel.app` atualizado. |
| 30/06/2026 | Fase 37 | Pedido do usuário ("falta o teste automatizado que ficou para trás"), depois ampliado explicitamente ("quero cobertura bem mais ampla agora") após eu confirmar que o escopo original cobria só 3 pontos recorrentes. Projeto não tinha nenhum framework de teste. Instalado **Vitest** (`vitest.config.ts` com alias `@/`→`src/`, scripts `npm test`), testes em `tests/**/*.test.ts`. Estratégia: mockar `@/lib/db` (`sql`) e dependências de rede (`fetch`, `@/lib/telegram`, `@/lib/whatsapp`) via `vi.mock`/`vi.stubGlobal`, com `await import()` dinâmico após configurar env vars/mocks (vários `lib/*.ts` lançam exceção no carregamento do módulo se a env var correspondente não estiver setada, ex.: `DATABASE_URL` em `lib/db.ts`, `JWT_SECRET` em `lib/auth.ts`). Funções puras que estavam presas em `route.ts` (não dá pra exportar livremente — quebraria a validação de Route Handler do Next) foram extraídas para `lib/`: `cortarNoFimDeFrase()` (Fase 31, corte de legenda no fim de frase real, ignorando abreviações) movida de `gerar-card/route.ts` para novo arquivo `lib/texto.ts`; `contemScriptNaoPortugues()` (Fase 19, filtro de script não-português) e `pick()` (Fase 25, seed de foto por id de notícia) só ganharam `export` nos arquivos `lib/` onde já moravam (`ai.ts`, `card-generator.tsx`). 9 arquivos de teste criados, 47 testes no total: `tests/lib/ai.test.ts` (7, validação de idioma), `tests/lib/whatsapp.test.ts` (4, formato achatado do payload Evolution API v2 + retry — regressão direta das Fases 11/20), `tests/lib/cupons.test.ts` (6, `validarCupom` incl. claim atômico anti-corrida), `tests/lib/alertas.test.ts` (2, `criarAlertaDedup` anti-spam da Fase 17), `tests/lib/auth.test.ts` (7, JWT + comparação em tempo constante + fallback `CLAUDE_AUTOFIX_SECRET`→`CRON_SECRET` da Fase 21), `tests/lib/mrr.test.ts` (3, agregação JS de `calcularMRR`), `tests/lib/card-generator.test.ts` (5, seed de foto incl. wraparound negativo/grande), `tests/lib/texto.test.ts` (5, corte de legenda incl. abreviações). Para os 2 padrões recorrentes sem viabilidade de teste comportamental sem banco real — claim atômico de cron (`FOR UPDATE SKIP LOCKED`/`'__PROCESSANDO__'` em `bot-responder`/`publicar-noticias`/`resumir-noticias`/`resumir-noticias-global`) e payload `sendMedia` de `gerar-card/route.ts` (que importa `next/og`+`sharp`, não isolável em ambiente node de teste) — criados testes de regressão por inspeção do texto-fonte (`tests/regressao/claim-atomico.test.ts`, `tests/regressao/sendmedia-payload.test.ts`), que travam a presença dos padrões corretos contra reintrodução do bug por refactor futuro, mesmo não sendo testes comportamentais reais. `npm test`: 47/47 passando, 10 arquivos. `tsc --noEmit`: 0 erros novos (único remanescente é o mesmo pré-existente e não-relacionado de `admin/usuarios/[id]/route.ts`). Sem commit/push/deploy — aguardando autorização do usuário. |
| 29/06/2026 | Fase 36 | Bug urgente reportado pelo usuário com prints: mesma notícia publicada duas vezes no mesmo grupo (VIP e Elite), uma como texto às 19h17 BRT e outra como card às 19h34 BRT. Causa raiz: `publicar-noticias` e `gerar-card` usam flags completamente independentes (`postada_vip` vs `postada_vip_card`) — o pipeline de notícias em `alerta-patriota-noticias.yml` tem `sleep 30` entre cada step (coletar→curar→resumir→publicar), então o `publicar-noticias` real chega atrasado (~1h17min após o trigger das 21h UTC = 19h17 BRT). O `gerar-card` roda a cada 30min via `alerta-patriota-crons.yml` e na rodada das 22h30 UTC (19h30 BRT) encontrou a mesma história (recém-publicada como texto 17min antes) com `postada_vip_card=false` — sem saber que texto acabou de sair. Fix: adicionada condição em ambas as queries do `gerar-card` para excluir histórias publicadas como texto nos últimos 60 minutos (`postada_vip_at < NOW() - INTERVAL '60 minutes'`), sem alterar histórias que ainda não saíram como texto. `tsc --noEmit` validado. **Deploy:** Commit `2c61620`, push direto (sem commits do bot no remoto). `vercel --prod` executado com sucesso (build 41s) — deploy `dpl_9jb2qUHtshhyRJSJpGQMmpmctmTc` promovido a produção, alias `alertapatriota.vercel.app` atualizado. |
| 29/06/2026 | Fase 35 | Retomado o backlog de segurança/infra deferido desde as Fases 21-23 (interrompido pelo bug urgente da Fase 34), todos os 6 itens reconfirmados ainda presentes antes de corrigir. Item 1 (throttle entre crons WhatsApp): nova tabela `whatsapp_throttle` + claim atômico por plano em `enviarMensagemGrupo`, evita rajada de mensagens no mesmo grupo quando vários crons disparam quase juntos. Item 2 (rate limit em `Map`): `criar-pix`/`criar-direto` migrados para tabela `assinaturas_rate_limit`, mesmo padrão de `leads/registrar` (Fase 30/Item 26) — `Map` em memória não sobrevive a serverless. Item 3 (heartbeat sem monitoramento externo): código (`avisarDeadManSwitch()` em `agente-heartbeat`) + usuário criou conta no healthchecks.io e configurou `DEADMAN_SWITCH_URL` na Vercel — ativado e testado em produção (ping `200 OK`). Item 4 (prompt-injection RSS): `lib/ai.ts` ganhou `INSTRUCAO_ANTI_INJECAO` anexada a toda chamada de IA, mitigação central sem reescrever ~10 rotas que interpolam RSS no prompt. Item 5 (CSP ausente): `next.config.ts` ganhou `Content-Security-Policy` completa (confirmado via grep que o app não carrega script/iframe externo nenhum, só fonte do Google no admin). Item 6 (lockfile ausente): `package-lock.json` gerado via `npm install --package-lock-only`; achado lateral fora de escopo — 4 vulnerabilidades moderadas transitivas (`postcss`/`uuid`), só corrigíveis com downgrade major, não corrigidas. `tsc --noEmit` e `npm run build` completo validados, zero erros novos (único remanescente é o pré-existente de `admin/usuarios/[id]/route.ts`). Commit `3b718db` + merge bot `guardian-state` + push (`7890127..8c4c83d`) + `vercel --prod` (`dpl_7H9Ko5v6oRYC5fV6YTuy7TgKAcc2`) + `POST /api/admin/setup` chamado em produção logo após o deploy (tabelas confirmadas via `information_schema`). Item 3 ativado em seguida com 2º deploy (`dpl_4DebMMR3VKC2hW6r8sVAF3j8awGX`) após configurar a env var. |
| 29/06/2026 | Fase 34 | Usuário reportou bug urgente no meio da investigação do backlog de segurança/infra: ANÁLISE URGENTE duplicada no grupo Elite + conteúdo de políticos/empresários brasileiros indo para o Elite (grupo que deve ser só direita mundial/global). Causa 1 (duplicação): dedup só por `url` exata falhava quando a mesma declaração saía como vídeo YouTube + matéria de portal (URLs diferentes, mesmo evento) — corrigido com dedup adicional por pessoa+3h, batched no mesmo padrão de `contagensPorPessoa`. Causa 2 (conteúdo errado no Elite): Prof. Cavalcanti gerava "ângulo global" só no tom do texto, mas a lista `PESSOAS` monitorada era 100% brasileira — o Elite já é servido corretamente por `resumir-noticias-global.ts` (pipeline separado, conteúdo genuinamente internacional); por escolha do usuário, Cavalcanti/Elite removidos deste arquivo (função `gerarAlertaCavalcanti`, bloco de postagem no Elite, campo `tipo` e os 3 empresários da lista `PESSOAS`, que sem Cavalcanti não geravam mais post nenhum). `tsc --noEmit` zerado (único erro remanescente é o mesmo pré-existente e não-relacionado de `admin/usuarios/[id]/route.ts`). Commit `606d479` + merge bot `guardian-state` + push (`36f9ffa..5ff4917`) + `vercel --prod` (`dpl_6V2nKPnbYkLqumbhoYdAY2d3RyHp`) realizados com autorização do usuário. |
...
| 02/07/2026 | Fase 55 | Bruno Backup nunca criou branch no Neon desde 30/05/2026 (data de criação do projeto). Causa raiz: `NEON_API_KEY` e `NEON_PROJECT_ID` nunca foram adicionadas ao Vercel production. Fix em 2 partes: (55-a) código do `criar_branch_neon` atualizado para logar mensagem de erro real em vez de gravar `sucesso` com `branchCriado: false`; env vars adicionadas ao Vercel via CLI. (55-b) Erro persiste: BOM (`﻿`, char code 65279) no valor de `NEON_API_KEY` corrompeu o header HTTP `Authorization: Bearer ﻿ napi_...` — confirmado pelo erro de runtime no log do banco. Fix final: `.replace(/^﻿/, '').trim()` nas leituras de `NEON_API_KEY` e `NEON_PROJECT_ID` em `backup/route.ts`. Verificação: branch `backup-2026-07-02` (id `br-floral-voice-ap6kx2kn`) criada com sucesso às 22:58 BRT, confirmada via API do Neon (`current_state: ready`, `parent_lsn: 0/AFB83A0`). Commits `ff06c41` (55-a) e `e984274` (55-b). |
| 13/07/2026 | Fase 56 | Remoção total do Anthropic dos 2 últimos pontos que ainda pagavam por padrão — pedido explícito do usuário após relatório de auditoria de custo de API (`relatorio-uso-api-anthropic.md`), com decisão confirmada via pergunta direta em ambos os casos: "Não quero que usa mais nada do Antropic e troque pelo Groq - Cerebras" (ai-helper.cjs) e "Remover Anthropic totalmente" (claude-revisor) — não foi um simples reordenar de cascata, e sim eliminação completa da camada paga, mesmo sabendo do histórico de incidente do claude-revisor (Fases 7/19-20/06). **Correção a um relatório anterior:** `revisor-seguranca/route.ts` já usa `gerarTexto()` (a cascata padrão Groq→Cerebras→Anthropic-last-resort de 21 rotas), não `gerarCodigoComClaude()` — não era de fato um dos pontos "pagos por padrão" e ficou fora do escopo, sem alteração. **1) `lib/ai.ts`:** lógica de request/validação de Groq e Cerebras (antes inline em `gerarTexto`) extraída para helpers privados `tentarGroq()`/`tentarCerebras()`, reaproveitados tanto por `gerarTexto()` (cascata das 21 rotas + revisor-seguranca, comportamento inalterado, ainda cai no Anthropic via `gerarComAnthropicComDisjuntor` como último recurso) quanto pela nova `gerarCodigoComClaude()` (usada só pelo `claude-revisor`), que agora tenta Groq → Cerebras e, se ambos falharem, lança erro combinado — sem nenhuma camada Anthropic. Disjuntor do Anthropic mantido intacto para `gerarTexto`. **2) `claude-revisor/route.ts`:** só comentários — bloco de topo e o aviso do incidente 19-20/06 atualizados para deixar claro que a rede de segurança real sempre foi `validarAntesDeCommitar()` (compile TS, cerca markdown, guard de encolhimento), não o provedor; nenhuma mudança de lógica, a chamada a `gerarCodigoComClaude(...)` mantém a mesma assinatura. **3) `automation/ai-helper.cjs`:** reescrito por completo — SDK do Anthropic, client e leitura de `ANTHROPIC_API_KEY` removidos; adicionado `CEREBRAS_API_KEY` + `MODELO_LLAMA_CEREBRAS`; helper `gerarComGroq` generalizado em `gerarComProvedorCompativel()` reaproveitado por Groq e Cerebras; portada a checagem de vazamento de script não-português (`REGEX_SCRIPT_NAO_PORTUGUES`) que já existia em `lib/ai.ts`; `gerarTexto()` agora tenta Groq → Cerebras → erro combinado. `whatsapp-dossie.cjs` (único consumidor, roda só sábado) não precisou de nenhuma alteração — mesma assinatura de chamada. `@anthropic-ai/sdk` em `automation/package.json` mantido pois `instagram-responder.cjs` (job desativado) ainda o referencia. **4) `alerta-patriota-crons.yml`:** job `dossie-elite` — `ANTHROPIC_API_KEY` removida do `env:`, `CEREBRAS_API_KEY: ${{ secrets.CEREBRAS_API_KEY }}` adicionada. **5) Secret `CEREBRAS_API_KEY`:** não existia no repo GitHub (`gh secret list` só mostrava `ANTHROPIC_API_KEY`/`GROQ_API_KEY` nesse job); criado via `gh secret set` a partir do valor funcional em `.env.local`. **Falso alarme investigado e descartado:** ao puxar o env de produção da Vercel para esse passo, `CEREBRAS_API_KEY` veio vazio (`""`) via `vercel env pull`, levantando suspeita de que a 2ª camada da cascata das 21 rotas estivesse silenciosamente inativa. Investigação (autorizada pelo usuário) mostrou que **todas** as env vars do projeto — inclusive `GROQ_API_KEY` e `ANTHROPIC_API_KEY`, comprovadamente funcionais em produção pelo histórico de fases anteriores — também retornam vazias em `vercel env pull` neste projeto; é uma limitação do `vercel env pull`/CLI em não expor valores de volta, não um sinal de valor ausente. Ou seja, a cascata Groq→Cerebras→Anthropic nunca esteve quebrada. Mesmo assim, como o usuário autorizou a correção, o valor de `CEREBRAS_API_KEY` em produção foi removido e recriado via API REST da Vercel (`PATCH /v9/projects/{id}/env/{envId}`) com o valor confirmado de `.env.local`, sem qualquer regressão. `tsc --noEmit`: 0 erros. `git diff --stat`: 4 arquivos, 116 inserções, 66 deleções. **Deploy:** usuário autorizou ("commit + push + deploy agora"). Commit `82ed7f2` (rebase sobre commits automáticos de outros squads sem conflito, push `196dd82..3d2ba60`). `vercel --prod` executado com sucesso — deploy `dpl_9gUs82Xo9bZfy41syNA74dWQX4CA` promovido a produção, alias `alertapatriota.vercel.app` atualizado. Workflow `alerta-patriota-crons.yml` (job `dossie-elite`) já está com `CEREBRAS_API_KEY` no push, efetivo no próximo disparo (sábado). |
| 13/07/2026 | Fase 57 (investigação) | **Usuário reportou:** VIP e Elite não estão recebendo notícias em forma de mensagem no WhatsApp. **Investigação (sem DB direto — `neon()` HTTP driver não alcança `api.c-7.us-east-1.aws.neon.tech` a partir desta máquina local, `ConnectTimeoutError`; usado em vez disso `mcp__claude_ai_Vercel__get_runtime_errors`/`get_runtime_logs` + chamada direta ao endpoint de diagnóstico já em produção):** **Achado principal — Evolution API fora do ar:** chamado `GET https://alertapatriota.vercel.app/api/cron/fiscal-whatsapp` (rodando em produção na Vercel, não localmente) → `503 { "ok": false, "erro": "Error: Status 404" }`. Ou seja, a própria Vercel, ao checar `${EVOLUTION_API_URL}/instance/connectionState/alertapatriota` (host `evolution-api-v2-production-5971.up.railway.app`, migrado na Fase 20), recebe 404 da Evolution API. Isso quebra tanto `gerar-card` (cards de imagem, `sendMedia`) quanto `publicar-noticias` (texto), os dois dependem do mesmo host/instância para o envio real. Sem acesso a token/API do Railway neste ambiente para inspecionar o serviço diretamente — usuário optou por fornecer acesso ao Railway numa próxima etapa. **Achado secundário — `publicar-noticias` nunca esteve agendado:** confirmado via grep no `alerta-patriota-crons.yml` que a string `publicar-noticias` não aparecia em lugar nenhum do workflow — o endpoint só era acionável manualmente via "Publicar agora" no admin, nunca rodava sozinho. **Achado descartado como causa principal:** erro "Task timed out after 60 seconds" recorrente desde 07/06/2026 (count=282 em 14 dias, `get_runtime_errors`) afeta 10 rotas de IA incluindo `gerar-card` — mas a taxa isolada de `gerar-card` é de só 1 timeout em 7 dias (`get_runtime_logs` agrupado por rota), não explica falha total; confirmado também (diff `git diff a18858e 3d2ba60 -- lib/ai.ts`) que o refactor da Fase 56 não alterou a lógica de retry/timeout de `gerarComOpenAICompativel`, então não é regressão introduzida por aquela fase. **Correção aplicada nesta fase (autorizada pelo usuário):** agendado `publicar-noticias` no `alerta-patriota-crons.yml` — novo cron `0 10,16,22 * * *` (7h/13h/19h BRT, mesmo horário do comentário original do código) + novo job `publicar-noticias` (chama `?grupo=vip` e, após `sleep 10`, `?grupo=elite`, mesmo padrão do job `gerador-cards`). YAML validado com `python -c "import yaml; yaml.safe_load(...)"` → OK. **Pendente, aguardando usuário:** acesso/token do Railway para investigar e reconectar a Evolution API — sem isso, mesmo com `publicar-noticias` agendado, os envios devem continuar falhando até a Evolution API voltar a responder. Nenhum commit/push/deploy feito ainda nesta fase — aguardando o usuário fornecer o acesso ao Railway antes de fechar o ciclo completo (fix + verificação real de envio). |
