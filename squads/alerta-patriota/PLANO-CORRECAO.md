...
**Deploy:** aguardando autorização do usuário. **Nota:** o deploy materializa as tabelas novas (`whatsapp_throttle`, `assinaturas_rate_limit`) só depois de `POST /api/admin/setup` ser chamado manualmente em produção (mesmo passo final já feito em fases anteriores, ex: Fase 33/Item 5). **Atenção ao tipo de falha nesse intervalo:** em `whatsapp.ts`, a query do throttle tem `.catch(() => [])` e simplesmente não aplica o atraso se a tabela não existir (degrada bem). Já em `criar-pix`/`criar-direto`, o `SELECT`/`INSERT` em `assinaturas_rate_limit` não tem `.catch()` — se a tabela ainda não existir, a exceção sobe e a rota responde HTTP 500 ("Erro interno"), bloqueando o checkout até o `admin/setup` rodar. É o mesmo padrão (sem proteção) já usado em `leads/registrar` desde a Fase 30, não é um risco novo deste lote — mas por afetar checkout pago, o `admin/setup` deve ser chamado imediatamente após o deploy, sem demora, para minimizar essa janela.
