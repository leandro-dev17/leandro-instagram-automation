...
**Item 3 — Heartbeat sem monitoramento externo: ⚠️ PARCIAL — código pronto, requer ação do usuário**

Confirmado: `agente-heartbeat/route.ts` só alertava via Telegram — se o próprio GitHub Actions parasse de disparar este cron (já aconteceu, Fase 15), ninguém seria avisado, porque o aviso depende do cron rodar. Implementado `avisarDeadManSwitch()`: faz `fetch` pra `process.env.DEADMAN_SWITCH_URL` logo após o `enviarTelegram` de sucesso; sem a env var configurada, a função não faz nada (zero mudança de comportamento até a ativação). **Pendente do usuário:** criar uma conta grátis num serviço de dead man's switch (ex: healthchecks.io, cronitor.io) e configurar a URL de ping gerada como `DEADMAN_SWITCH_URL` na Vercel — sem isso, o item continua sem proteção real, só o código está pronto para quando a env var existir.
